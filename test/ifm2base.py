...
def func(param,point):
    ...
    """
    平面拟合误差函数，其实就是平面方程,B固定为零，不参与计算,A固定为-1
    """
    C, D = param
    A=-1
    xx= point[0]
...
def callback_kinect(cloud_in):
    ...
    #    pub_cloud.publish(cloud_out)
       if kinect_tf != None:
        ...
        num,nums=0,[]
        for p in data_out:
            # if p[0]<2.0 and p[0]>1.0 and p[1]>-1.0 and p[1]<1.0:
            if p[0]<1.5 and p[0]>0.5 and p[1]>-0.5 and p[1]<0.5:
                if num==0:
                    nums.append(p)
                    num += 1
        ...
        if num_calc>200:
            ...
            transform_body.child_frame_id = "/o3d3xx/camera_link"
            M=np.identity(4)
            M[:3,:3]=tf_rot
            q=tf.transformations.quaternion_from_matrix(M)
            ...
            fp4=open("%s/ifm2base.data"%(file_path),'a+')
...
def init():
    ...
    rospy.Subscriber("/o3d3xx/camera/cloud", PointCloud2, callback_kinect)
