...
def callback_ifm(cloud_in):
        ...
        fp4.write('     x          y            z        instensity\n')
        nums=0
        for p in data_out:
            # nums.append(p)
            fp4.write("%-12.6f %-12.6f %-12.6f %-12.6f\n" %(p[0],p[1],p[2],p[3]))
            nums+=1
...
def init():
    ...
    fp4=open("%s/points-ifm.txt"%(file_path),'a+')

    rospy.init_node("sensor", anonymous=True)
    rospy.Subscriber("/o3d3xx/camera/cloud", PointCloud2, callback_ifm)
